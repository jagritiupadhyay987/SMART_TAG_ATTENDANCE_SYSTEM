...
create_install_dir() {
    local bin_dir="${CODERABBIT_INSTALL_DIR:-$HOME/.local/bin}"
...
get_version() {
    ...
    local version_url="${base_url}/latest/VERSION"
    local version
...
install_cli() {
    ...
    local base_url="${CODERABBIT_DOWNLOAD_URL:-https://cli.coderabbit.ai/releases}"
    local version=$(get_version "$base_url")
    local download_url="${base_url}/${version}/coderabbit-${OS}-${ARCH}.zip"
    local temp_dir
    local temp_file
    local install_path="$BIN_DIR/coderabbit"
    ...
    local binary_path="$temp_dir/coderabbit"
